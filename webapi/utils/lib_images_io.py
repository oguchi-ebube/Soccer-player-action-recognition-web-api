...
class VideoWriter(object):
    ...
    def write(self, img):
        ...
        if self._cnt_img == 1:  # initialize the video writer
            ...
            self._video_writer = cv2.VideoWriter(self._video_path, fourcc, self._framerate, (self._width, self._height))
            
               
        self._video_writer.write(img)
        cv2.imwrite(api.config["BASE_DIR"]+"/"+"webapi/static/video_output/file.png",img)
        # socketio.emit("image_frame","/static/video_output/file.png")
